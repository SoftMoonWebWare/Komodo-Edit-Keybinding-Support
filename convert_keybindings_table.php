...
<head>
<meta charset='UTF-8'>
<meta name='x-generator' content="SoftMoon-WebWare's convert_keybindings_table.php  http://softmoon-webware.com/Komodo/">
...
body.flow .window_pane {
	...
	width: 100%;
	height: auto; }
body.sideby .window_pane { /*can confuse MSIE if aside-nav is before the main sections*/
	position: fixed;
	top: 1.2em;
	left: 50%;
	right: auto;
	...
	width: 50%;
	overflow: auto; }
body.sideby header + .window_pane {
	left: 0;
	border-right: 2px double; }
body.stack .window_pane { /*can confuse MSIE if aside-nav is before the main sections*/
	position: fixed;
	top: 50%; /*for Presto*/
	top: calc(50% + .6em);
	left: 0;
...
body.stack header {
	...
	border-right: 2px double;
	z-index: 6180; }
body.sideby header div,
body.stack header div {
	display: block;
	position: absolute;
	top: 2em;
	left: -1.382em;
...
<meta name='last_update' content='March 26, 2014' />
	...
	http://localhost/convert_keybindings_table.php?groomcats=false  ←does not show keybindings by category
...
$myKeybindings=new KomodoKeybindingSupport(strToLower($_GET['groomcats'])!=='false');

if (isset($_GET['filename']))  {
	if (is_file("Komodo Keybindings [".$_GET['filename']."].htm"))
		$_GET['filename']="Komodo Keybindings [".$_GET['filename']."].htm";  }
else  $_GET['filename']='keybindings_by_category.htm';   // filename of user-saved HTML generated by Komodo

$myKeybindings->parse_cat_file($_GET['filename'], $_GET['filter']);
...
if (isset($_GET['compare']))  {
	...
	$myKeybindings->parse_cat_file($_GET['compare'], $_GET['filter']);  }
...
public $cat_file, $eol, $charset,
			 ...
			 $snippets=0, $macros=0, $URLs=0, $commands=0,
			 $HTML=array('keys'=>"", 'cats'=>"");
private $¿groom_cats, $filecount=0,
				$order_marks="/^[αβγδεζηθικλμν•]+•/u",  //note the PHP-specific “u” modifier for UTF-8 processing
				$doubles=array('α•1 !', 'α•2 @', 'α•3 #', 'α•4 $', 'α•5 %', 'α•6 ^', 'α•7 &', 'α•8 *', 'α•9 (', 'β•0 )',
											 'γ•- _', 'δ•= +', 'ε•[ {', 'ζ•] }', 'η•\\ |', 'θ•` ~', 'ι•\' "', 'κ•; :', 'λ•, <', 'μ•. >', 'ν•/ ?');
/* Notes: (1) The $doubles array above is for a standard American-style keyboard used with MS-Windows®.
 ...
 * The “build_HTML” method specifically removes these Greek glyphs from keys when found at the beginning
 * of a key and when postfixed with a bullet • (see $order_marks above).  This should be compatible with a
...
public function parse_cat_file($cat_file, $filter)  { $this->filecount++;
	$this->cat_file=file_get_contents($cat_file);
	preg_match('/\n\r|\n/', $this->cat_file, $eol);
	...
	for ($i=0, $c=count($F);  $i<$c;  $i++)  { $o=0;
		...
		while (preg_match('/<p>([^<]+)<\/p>/u', $F[$i], $m, PREG_OFFSET_CAPTURE, $o))  {
			$o=$m[1][1]+strLen($m[1][0]);
			if ($binding)  {
				if ($this->¿groom_cats)  $this->augment_cats($cat, $binding[0], $m[1][0]);
...
<?php
	for ($i=1, $c=count($this->headline);  $i<=$c;  $i++)  {
		echo "<h1>",($this->filecount>1 ? "<span class='file$i'>$i</span>" : ""),$this->headline[$i-1]."</h1>\n";  }
	echo "<p>",$this->snippets," total snippet keybindings</p>\n",
			 "<p>",$this->macros," total macro keybindings</p>\n",
			 "<p>",$this->URLs," total URL keybindings</p>\n",
			 "<p>",$this->commands," total command keybindings</p>\n";  ?>
</header>
...
<?php  } //close  echo_HTML  method


public function rgb_from_hcg($hue, $chroma, $gray)  {
	// hue (pure color from color wheel)
	// chroma (how much color to mix into the gray: 0=no color, pure gray …… 1=pure color, no gray)
	...
	if ($hue<2)  {$red=1-$x; $grn=1; $blu=0;}
	else
	if ($hue<3)  {$red=0; $grn=1; $blu=$x;}
	else
	if ($hue<4)  {$red=0; $grn=1-$x; $blu=1;}
	else
